...
func tambahPabrikan(data *[MAX_DATA]Pabrikan, jumlah *int) {
	if *jumlah >= MAX_DATA {
		fmt.Println("Data pabrikan penuh!")
		return
	}
...
func updatePabrikan(data *[MAX_DATA]Pabrikan, jumlah int) {
	if jumlah == 0 {
		fmt.Println("Belum ada data pabrikan yang tersedia!")
		return
	}
	...
	for i := 0; i < jumlah; i++ {
		if data[i].ID == id {
			...
			fmt.Print("Masukkan Negara Pabrikan: ")
			fmt.Scanln(&data[i].Negara)

			fmt.Print("Masukkan Tahun Berdiri: ")
			fmt.Scanln(&data[i].TahunBerdiri)

			fmt.Print("Masukkan Alamat Pabrikan: ")
			fmt.Scanln(&data[i].Alamat)
